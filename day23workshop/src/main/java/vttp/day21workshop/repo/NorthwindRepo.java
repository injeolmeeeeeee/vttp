...
import vttp.day21workshop.model.OrderDetails;
    ...
    public OrderDetails getOrderDetailsByOrderId(String orderId) {
        try {
            SqlRowSet rowSet = template.queryForRowSet(
                    Queries.SQL_GET_ORDER_BY_ORDER_ID,
                    new Object[] { orderId });
            if (rowSet.next()) {
                OrderDetails orderDetails = new OrderDetails();
                orderDetails.setOrderId(rowSet.getString("order_id"));
                orderDetails.setOrderDate(rowSet.getString("order_date"));
                orderDetails.setCustomerId(rowSet.getString("customer_id"));
                orderDetails.setTotal(rowSet.getDouble("total"));
                orderDetails.setCostPrice(rowSet.getDouble("cost_price"));
                return orderDetails;
            } else {
                return null;
            }
